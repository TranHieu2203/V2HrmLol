...
{
    public class FormListRepository : TLARepository<FormList>, IFormListRepository
    {
        ...
        public async Task<ResultWithError> GetById(int id)
        {
            try
            {
                ...
                while (id >= 10) id /= 10;
                var x = await QueryData.ExecuteStore<ReferParam>("PKG_COMMON.FORM_ELEMENT", new
                {
                    P_TYPE_ID = id,
        ...
        public async Task<ResultWithError> PrintFormSalary(PayrollInputDTO param)
        {
            try
            {
                var data = await QueryData.ExecuteStoreObject("PKG_COMMON.FORM_PRINT_SALARY",
                    new
                    {
        ...
        public async Task<ResultWithError> PrintFormAttendance(PayrollInputDTO param)
        {
            try
            {
                var data = await QueryData.ExecuteStoreObject("PKG_COMMON.FORM_PRINT_ATTENDANCE",
                    new
                    {
                        P_IS_ADMIN = _appContext.IsAdmin == true ? 1 : 0,
                        P_ORG_ID = param.OrgId,
                ...
                foreach (var item in listRemind)
                {
                    switch (item.Code)
                    {
                        case SystemConfig.EMP_BIRTH_DAY:
                            var x = await (from p in _appContext.Employees.Where(c => c.BIRTH_DATE != null)
                                           where ((DateTime)p.BIRTH_DATE).DayOfYear - d <= item.Day && ((DateTime)p.BIRTH_DATE).DayOfYear - d >= 0
        ...
        public async Task<ResultWithError> UpdateRemind(List<SettingRemindDTO> param)
        {
            try
            {
                ...
                var list = new List<SettingRemind>();
                for (int i = 0; i < param.Count; i++)
                {
                    if (param[i].IsActive == true && param[i].Day != null)
                    {
                        var r = Map<SettingRemindDTO, SettingRemind>(param[i], new SettingRemind());
                        list.Add(r);
                    }
        ...
        public async Task<ResultWithError> GetDashboard()
        {
            try
            {
                var r = await QueryData.ExecuteStoreObject("PKG_DEMO.STATISTIC_COMMON",
                new
                {
                    P_TENANT_ID =  0,
        ...
        public async Task<ResultWithError> REPORT_HU001()
        {
            try
            {
                var r = await QueryData.ExecuteStoreObject("PKG_REPORT.REPORT_HU001",
                new
                {
                    P_IS_ADMIN = _appContext.IsAdmin == true ? 1 : 0,
        ...
        public async Task<ResultWithError> REPORT_HU009()
        {
            try
            {
                var r = await QueryData.ExecuteStoreObject("PKG_REPORT.REPORT_HU009",
                new
                {
                    P_IS_ADMIN = _appContext.IsAdmin == true ? 1 : 0,
                    P_DATE = QueryData.OUT_CURSOR,
