...
{
    public class VoteRepository : TLARepository<Answer>, IVoteRepository
    {
        ...
        public async Task<ResultWithError> AddAnswer(QuestionOutputDTO param)
        {
            ...
            if (q.EXPIRE != null && System.DateTime.Now > q.EXPIRE)
            {
            ...
            if (param.Answers != null && param.Answers.Count > 0)
            {
                ...
                foreach (var item in param.Answers)
                {
                    ...
                    {
                        var aswerNew = Map<AnswerOutputDTO, Answer>(item, new Answer());
                    ...
                    else  // vote nhung cau tra loi cu
                    {
                        ...
                        {
                            _appContext.AnswerUsers.Add(new AnswerUser() { EMP_ID = _appContext.EmpId, ANSWER_ID = (int)item.Id });
                        }
        ...
        public async Task<ResultWithError> CreateQuestion(QuestionDTO param)
        {
            ...
            try
            {
                var data = Map<QuestionDTO, Question>(param, new Question());
                data.IS_ACTIVE = true;
                ...
                if (param.Answers != null && param.Answers.Count > 0)
                {
                    ...
                    foreach (var item in param.Answers)
                    {
                        var answer = Map<AnswerDTO, Answer>(item, new Answer());
        ...
        public async Task<ResultWithError> PortalList()
        {
            try
            {
                var r = await QueryData.ExecuteStore<PortalVoteParam>("PKG_PORTAL.VOTE_LIST", new
                {
