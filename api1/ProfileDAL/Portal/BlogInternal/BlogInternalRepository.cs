...
{
    public class BlogInternalRepository : TLARepository<BlogInternal>, IBlogInternalRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(BlogInternalInputDTO param)
        {
            ...
            try
            {
                var data = Map<BlogInternalInputDTO, BlogInternal>(param, new BlogInternal());
                data.IS_ACTIVE = true;
        ...
        public async Task<ResultWithError> UpdateAsync(BlogInternalInputDTO param)
        {
            ...
            var data = Map<BlogInternalInputDTO, BlogInternal>(param, r);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<ResultWithError> PortalGetById(int id)
        {
            var r = await (from p in _appContext.BlogInternals
                           where p.ID == id 
                           select new
        ...
        public async Task<List<NotifyView>> PortalNotify()
        {
            ...
            try
            {
                var r = await QueryData.ExecuteStore<NotifyView>("PKG_PROFILE.GET_NOTIFY", new
                {
        ...
        public async Task<ResultWithError> PortalWatched(long id)
        {
            ...
            // emp-notification
            List<int> lstEmpNoti = r.EMP_NOTIFY_ID.Split(",").Select(int.Parse).ToList();
            // is_read
        ...
        public async Task<ResultWithError> PortalApproveNotify()
        {
            try
            {
                var r = await QueryData.ExecuteStore<NotifyApproveView>("PKG_NOTIFY.PORTAL_COUNT_REG", new
                {
