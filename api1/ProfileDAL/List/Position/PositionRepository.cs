...
{
    public class PositionRepository : TLARepository<Position>, IPositionRepository
    {
        ...
        public async Task<PagedResult<PositionViewDTO>> GetAll(PositionViewDTO param)
        {
            var queryable = from p in _appContext.Positions
                            ...
                            from m in _appContext.Employees.Where(x => x.ID == (long?)p.MASTER).DefaultIfEmpty()
                            from i in _appContext.Employees.Where(x => x.ID == (long?)p.INTERIM).DefaultIfEmpty()
                            ...
                            select new PositionViewDTO
                            {
                                ...
                                orgname = o.NAME,
                                master = p.MASTER,
                                mastername = m.CODE + " - " + m.FULLNAME,
                                interim = p.INTERIM,
                                interimname = i.CODE + " - " + i.FULLNAME,
                                lm = p.LM,
                                lmname = tt.FULLNAME,
                                jobid = p.JOB_ID,
                                jobname = j.NAME_VN
        ...
        public async Task<ResultWithError> GetById(int id)
        {
            try
            {
                var r = await (from p in _appContext.Positions
                               ...
                               from tt in _appContext.Employees.Where(f => f.ID == p.LM).DefaultIfEmpty()
                               from pd in _appContext.Employees.Where(f => f.ID == p.CSM).DefaultIfEmpty()
                               ...
                               from tt_pos in _appContext.Positions.Where(f => f.ID == tt.POSITION_ID).DefaultIfEmpty()
                               from pd_pos in _appContext.Positions.Where(f => f.ID == pd.POSITION_ID).DefaultIfEmpty()
                               where p.ID == id
                               select new PositionOutputDTO
                               {
                                   ...
                                   csm = p.CSM,
                                   csmname = pd.FULLNAME,
                                   csmposition = pd_pos.NAME,
                                   interim = p.INTERIM,
                                   interimname = i.CODE + " - " + i.FULLNAME,
                                   lm = p.LM,
                                   lmname = tt.FULLNAME,
                                   lmposition = tt_pos.NAME,
                                   filename = p.FILENAME,
        ...
        public async Task<ResultWithError> CreateAsync(PositionInputDTO param)
        {
            try
            {
                ...
                // }
                var data = Map<PositionInputDTO, Position>(param, new Position());
                data.IS_ACTIVE = -1;
                ...
                if (data.ID > 0)
                {
                    var dataDesc = Map<PositionDesriptionInputDTO, PositionDesription>(param._positionDesc, new PositionDesription());
                ...
                //return data
                var output = new PositionOutputDTO();
                var values = Map<Position, PositionOutputDTO>(data, output);
        ...
        public async Task<ResultWithError> UpdateAsync(PositionInputDTO param)
        {
            try
            {
                ...
                var data = Map<PositionInputDTO, Position>(param, a);
                var result = _appContext.Positions.Update(data);
                if (a_desc != null)
                {
                    var dataDesc = Map<PositionDesriptionInputDTO, PositionDesription>(param._positionDesc, a_desc);
        ...
        public async Task<ResultWithError> Delete(List<int> Ids)
        {
            try
            {
                ...
                List<PositionDesription> PoDecss = new List<PositionDesription>();
        ...
        public async Task<ResultWithError> GetByOrg(int orgId, int empId)
        {
            try
            {
                var data = await (from p in _appContext.Positions
                                  ...
                                  where p.IS_ACTIVE.Value != 0 && p.ORG_ID == orgId && p.IS_PLAN.Value
                                  && ((p.MASTER.HasValue && p.MASTER != empId && !p.INTERIM.HasValue) || !p.MASTER.HasValue || (p.MASTER.HasValue && p.MASTER == empId))
                                  select new
                                  {
                                      id = p.ID,
                                      name = p.CODE + " - " + p.NAME,
                                      master = (!string.IsNullOrEmpty(m.CODE) ? m.CODE + " - " + m.FULLNAME : ""),
                                      interim = (!string.IsNullOrEmpty(i.CODE) ? i.CODE + " - " + i.FULLNAME : "")
                                  }).ToListAsync();
                return new ResultWithError(data);
            }
            catch (Exception ex)
        ...
        public async Task<ResultWithError> GetLM(int positionId)
        {
            try
            {
                var data = await (from p in _appContext.Positions
                                  ...
                                  {
                                      id = p.ID,
                                      direcManagerPositionName = (!string.IsNullOrEmpty(m.CODE) ? m.CODE + " - " + m.NAME : ""),
                                      direcManagerName = (!string.IsNullOrEmpty(e.CODE) ? e.CODE + " - " + e.FULLNAME : "")
                                  }).FirstOrDefaultAsync();
                return new ResultWithError(data);
        ...
        public async Task<ResultWithError> ModifyPositionById(PositionInputDTO param, int orgRight, int Address, int orgIDDefault = 1, int isDissolveDefault = 0)
        {
            try
            {
                ...
                // Check quyen du lieu
                List<int?> objChk = (from p in _appContext.Positions
                ...
                ids = orgIds.Where(f => (int?)((dynamic)f).ID == orgRight).Select(c => (int?)((dynamic)c).ID).ToList();
                objChk = ids;
                ...
                if (param.isowner == true)
                {
                    ...
                    if (param.isPlanLeft == false)
                    {
                        ...
                        if (OrgParentLeft != null)
                        {
                            var PositionId = (from t in _appContext.Positions
                                              where t.ORG_ID == OrgParentLeft && t.ISOWNER == true && t.IS_ACTIVE == -1 && t.IS_PLAN == false
                            ...
                            if (PositionId != 0)
                            {
                                ...
                                var lstPositionChildLeft = (from d in _appContext.Positions
                                                            where (d.LM == param.id || d.CSM == param.id) && d.IS_ACTIVE == -1
                                                            select d).ToList();
                    ...
                    {
                        var PositionId = (from t in _appContext.Positions
                                          where t.ORG_ID == OrgParent && t.ISOWNER == true && t.IS_ACTIVE == -1 && t.IS_PLAN == false
                                          select t.ID).FirstOrDefault();
                        if (PositionId != 0)
                        {
                    ...
                    if (param.orgid != orgRight)
                    {
                        ...
                        lstPosition = (from t in _appContext.Positions
                                       where t.ORG_ID == OrgIdRight && t.ISOWNER == false && t.IS_ACTIVE == -1
                                       select t).ToList();
                            ...
                            if (lstOrg.Count > 0)
                            {
                                foreach (var it in lstOrg)
                                {
                                    var lstPositionChild = (from d in _appContext.Positions
                                                            where d.ORG_ID == it && d.ISOWNER == true && d.IS_ACTIVE == -1
                                                            select d).ToList();
                        ...
                        if (param.isowner == true && param.isplan == true)
                        {
                            ...
                            if (OrgParentA != null)
                            {
                                var PositionLeftParent = (from t in _appContext.Positions
                                                          where t.ORG_ID == OrgParentA && t.ISOWNER == true && t.IS_PLAN == false && t.IS_ACTIVE == -1
                                                          select t.ID).FirstOrDefault();
                ...
                else
                {
                    ...
                    if (param.isPlanLeft == false)
                    {
                        ...
                        var PositionParentLeft = (from d in _appContext.Positions
                                                  where d.ORG_ID == param.orgid && d.ISOWNER == true && d.IS_PLAN == false && d.IS_ACTIVE == -1
                        ...
                        else
                        {
                            ...
                            {
                                var PositionLeftParent = (from t in _appContext.Positions
                                                          where t.ORG_ID == OrgParent && t.ISOWNER == true && t.IS_PLAN == false && t.IS_ACTIVE == -1
                                                          select t.ID).FirstOrDefault();
                                if (PositionLeftParent != 0)
                                {
                                    // Update QLTT, QLPD của tất cả các Pos có nó là QLTT hoặc QLPD bằng Pos trưởng Org cha
                    ...
                    var PositionId = (from d in _appContext.Positions
                                      where d.ORG_ID == OrgIdRight && d.ISOWNER == true && d.IS_PLAN == false && d.IS_ACTIVE == -1
                                      select d.ID).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        if (OrgRightParent != null)
                        {
                            var PositionIdParent = (from d in _appContext.Positions
                                                    where d.ORG_ID == OrgRightParent && d.ISOWNER == true && d.IS_PLAN == false && d.IS_ACTIVE == -1
                                                    select d.ID).FirstOrDefault();
                ...
                var r = await QueryData.ExecuteStore<PositionViewDTO>("PKG_OMS_BUSINESS.JOB_AUTO_UPDATE_POSITION_TEMP", null);
            ...
            if (_filter.orgIdSearch == null || _filter.orgIdSearch < 0 || _filter.orgId2Search == null || _filter.orgId2Search < 0)
            {
                ...
                var lst = (from t in _appContext.Positions
                           ...
                           select new PositionViewDTO
                           {
                               ...
                               interim = t.INTERIM,
                               interimname = (t.INTERIM == null && t.MASTER == null ? str : (t.INTERIM != null ? a.CODE + " - " + a.FULLNAME : str3)),
                               isowner = t.ISOWNER,
                               isplan = t.IS_PLAN,
                               flag = 1,
                               jobid = t.JOB_ID,
                               ...
                               mastername = (t.INTERIM == null && t.MASTER == null ? str : (t.HIRING_STATUS == -1 ? str1 : (t.CONCURRENT == -1 ? e.CODE + " - " + e.FULLNAME + str2 : (t.MASTER != null ? e.CODE + " - " + e.FULLNAME : str3)))),
                               color = "#4BA838"
                           });
                ...
                if (Id == null || Id < 0)
                {
                    ...
                    await (query = (from t in _appContext.Positions
                                    ...
                                    select new PositionOutputDTO
                                    {
                                        ...
                                        master = t.MASTER,
                                        mastercode = e.CODE,
                                        mastername = ReturnString(t.MASTER, t.HIRING_STATUS, e.FULLNAME, t.CONCURRENT),
                                        remark = e.FULLNAME,
                                        hiringStatus = t.HIRING_STATUS,
                                        concurrent = t.CONCURRENT,
                                        isowner = t.ISOWNER,
            ...
            try
            {
                if (OrgId > 0)
                {
                    var exits = from t in _appContext.Positions
                                where t.IS_PLAN == false && t.ISOWNER == true && t.IS_ACTIVE == -1 && t.ORG_ID == (int)OrgId
                                select t;
                ...
                {
                    var exits = from t in _appContext.Positions
                                where t.IS_PLAN == true && t.ISOWNER == true && t.IS_ACTIVE == -1 && t.ORG_ID == (int)OrgId
                                select t;
                    if (exits?.ToList().Count > 0)
                        return true;
                    return false;
        ...
        public async Task<ResultWithError> InsertPositionNB(PositionInputDTO obj, int OrgRight, int Address, int OrgIDDefault = 1, int IsDissolveDefault = 0)
        {
            ...
            orgIds = await QueryData.ExecuteList("PKG_COMMON.LIST_ORG",
                                                new
                                                {
                                                    P_IS_ADMIN = _appContext.IsAdmin == true ? 1 : 0,
                                                    P_ORG_ID = OrgRight,
            ...
            ids = orgIds.Select(c => (int?)((dynamic)c).ID).ToList();
            ...
            try
            {
                if (obj.isowner == true)
                {
                    ...
                    if (OrgParent != null)
                    {
                        ...
                        if (PositionId != 0)
                        {
                            ...
                            var data = Map<PositionInputDTO, Position>(obj, new Position());
                            var result = await _appContext.Positions.AddAsync(data);
                            await _appContext.SaveChangesAsync();
                            gID = data.ID;
                        }
                        ...
                        if (obj.isplan == false)
                        {
                            ...
                            if (lstOrg.Count > 0)
                            {
                                ...
                                {
                                    List<Position> lstPositionChild = (from d in _appContext.Positions
                                                                       where d.ORG_ID == item1 && d.ISOWNER == true && d.IS_ACTIVE == -1
                                                                       select d).ToList();
                                    if (lstPositionChild.Count > 0)
                                    {
                ...
                {
                    var PositionId = (from t in _appContext.Positions
                                      where t.ORG_ID == OrgIdRight && t.ISOWNER == true && t.IS_ACTIVE == -1 && t.IS_PLAN == false
                                      select t.ID).FirstOrDefault();
