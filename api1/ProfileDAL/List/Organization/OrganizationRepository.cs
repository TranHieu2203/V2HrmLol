...
{
    public class OrganizationRepository : TLARepository<Organization>, IOrganizationRepository
    {
        ...
        public async Task<ResultWithError> GetTreeView()
        {
            ...
            var data = await _appContext.Organizations.Include(c => c.Childs).ToListAsync();
            ...
            try
            {
                var r = await (from p in _appContext.Organizations
                               ...
                               select new OrganizationDTO
                               {
                                   ...
                                   ParentName = c2.NAME,
                                   PosName = t2.NAME,
                                   Avatar = e2.AVATAR == null ? "/assets/images/no-img.png" : e2.AVATAR
                               }).FirstOrDefaultAsync();
        ...
        public async Task<ResultWithError> CreateAsync(OrganizationInputDTO param)
        {
            try
            {
                ...
                param.Id = null;
                var data = Map<OrganizationInputDTO, Organization>(param, new Organization());
                var result = await _appContext.Organizations.AddAsync(data);
                await _appContext.SaveChangesAsync();
                param.Id = data.ID;
                // add phân quyền
                var o = new UserOrgani();
                o.ORG_ID = data.ID;
        ...
        public async Task<ResultWithError> UpdateAsync(OrganizationInputDTO param)
        {
            try
            {
                ...
                var data = Map<OrganizationInputDTO, Organization>(param, r);
            ...
            try
            {
                var query = await (from p in _appContext.Organizations
                                   ...
                                   select new
                                   {
                                       ...
                                       pid = p.PARENT_ID,
                                       pName = o2.NAME,
                                       orgManager = e2.FULLNAME,
                                       Image = e2.AVATAR == null ? SystemConfig.NO_IMG : e2.AVATAR,
        ...
        public async Task<ResultWithError> GetAllOrgChartPosition(OrgChartRptInputDTO param)
        {
            try
            {
                var positionItems = QueryData.ExecuteStoreToTable("PKG_OMS_REPORT.GET_ORGANIZATION_CHART",
                        new
                        {
        ...
        public async Task<ResultWithError> UpdateCreateRptJobPosHisAsync(JobPositionTreeInputDTO param)
        {
            ...
            {                
                ...
                var ACTIONIU = QueryData.ExecuteStoreObject("PKG_OMS_REPORT.IU_RPT_JOB_POS_HIS",
                        new
                        {P_ID = param.Id,
        ...
        public async Task<ResultWithError> GetJobChildTree(JobChildTreeInputDTO param)
        {
            try
            {
                var JobChildItems = QueryData.ExecuteStoreToTable("PKG_OMS_REPORT.GET_JOB_CHILD_TREE",
                        new
                        {
                            P_LANGUAGE = param.language,
