...
{
    public class EntitlementEditRepository : TLARepository<EntitlementEdit>, IEntitlementEditRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(EntitlementEditInputDTO param)
        {
            try
            {
                var data = Map<EntitlementEditInputDTO, EntitlementEdit>(param, new EntitlementEdit());
        ...
        public async Task<ResultWithError> UpdateAsync(EntitlementEditInputDTO param)
        {
            try
            {
                ...
                var data = Map<EntitlementEditInputDTO, EntitlementEdit>(param, r);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<ResultWithError> GetBy(int id)
        {
            try
            {
        ...
        public async Task<ResultWithError> ExportTemplate(ParaOrg param)
        {
            try
            {
                var ds = QueryData.ExecuteStoreToTable("PKG_PROFILE.LIST_EMPLOYEE",
                new
                {
                    P_IS_ADMIN = _appContext.IsAdmin == true ? 1 : 0,
                    P_ORG_ID = param.OrgId,
        ...
        public async Task<ResultWithError> ImportTemplate(EntitlementEditParam param)
        {
            try
            {
                ...
                var lst = new List<EntitlementEdit>();
                ...
                foreach (var item in param.Data)
                {
                    ...
                    else
                    {
                        try
                        {
                            dtl.YEAR = Int32.Parse(item.Year);
                        }
                        catch
                        {
                            error = true;
                    ...
                    else
                    {
                        try
                        {
                            dtl.MONTH = Int32.Parse(item.Month);
                        }
                        catch
                        {
                            error = true;
                ...
                {
                    var pathTemp = _appContext._config["urlTempEntitlement"];
                    var memoryStream = Template.FillTemp<EntitlementEditImp>(pathTemp, lstError);
                    return new ResultWithError(memoryStream);
                }
                else
                {
