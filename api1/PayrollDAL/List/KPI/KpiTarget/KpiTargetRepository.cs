...
{
    public class KpiTargetRepository : TLARepository<KpiTarget>, IKpiTargetRepository
    {
        ...
        public async Task<ResultWithError> UpdateAsync(KpiTargetInputDTO param)
        {
            try
            {
                ...
                var data = Map<KpiTargetInputDTO, KpiTarget>(param, r);
                data.CODE = r.CODE;
                if (param.ColId == null)
                {
                    data.COL_ID = null;
                ...
                await _appContext.SaveChangesAsync();
                var a = QueryData.Execute("PKG_PAYROLL_SETTING.ELEMENT_FORMULA_KPI_CHANGE",
                   new
                   {
                       P_COL_NAME = r.CODE,
