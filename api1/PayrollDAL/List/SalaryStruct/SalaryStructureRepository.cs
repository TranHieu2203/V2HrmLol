...
{
    public class SalaryStructureRepository : TLARepository<SalaryStructure>, ISalaryStructureRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(SalaryStructureInputDTO param)
        {
            try
            {
                ...
                var data = Map<SalaryStructureInputDTO, SalaryStructure>(param, new SalaryStructure());
                
                var result = await _appContext.SalaryStructures.AddAsync(data);
                await _appContext.SaveChangesAsync();
                _appContext.Database.CommitTransaction();
                var a = QueryData.Execute("PKG_PAYROLL_SETTING.ELEMENT_FORMULA_CHANGE",
                  new
                  {
        ...
        public async Task<ResultWithError> UpdateAsync(SalaryStructureInputDTO param)
        {
            try
            {
                ...
                var data = Map<SalaryStructureInputDTO, SalaryStructure>(param, r);
                var result = _appContext.SalaryStructures.Update(data);
                await _appContext.SaveChangesAsync();
                await QueryData.Execute("PKG_PAYROLL_SETTING.ELEMENT_FORMULA_CHANGE",
                   new
                   {
                       P_SALARY_TYPE = param.SalaryTypeId,
                       P_ELEMENT_ID = param.ElementId,
        ...
        public async Task<ResultWithError> Delete(int id)
        {
            try
            {
                await QueryData.Execute("PKG_PAYROLL_SETTING.STRUCT_REMOVE_ELEMENT",
                                  new
                                  {
                                      P_ID = id
                                  }, true);
