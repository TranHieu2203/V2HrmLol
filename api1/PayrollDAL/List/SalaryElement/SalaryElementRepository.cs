...
{
    public class SalaryElementRepository : TLARepository<SalaryElement>, ISalaryElementRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(SalaryElementInputDTO param)
        {
            try
            {
                ...
                // 
                var data = Map<SalaryElementInputDTO, SalaryElement>(param, new SalaryElement());
                data.IS_ACTIVE = true;
                var result = await _appContext.SalaryElements.AddAsync(data);
                await _appContext.SaveChangesAsync();
                // Them column
                dynamic a = await QueryData.ExecuteObject("PKG_PAYROLL.ADD_ELEMENT_SAL", new
                {
        ...
        public async Task<ResultWithError> UpdateAsync(SalaryElementInputDTO param)
        {
            try
            {
                ...
                //}
                var data = Map<SalaryElementInputDTO, SalaryElement>(param, r);
                data.CODE = r.CODE;
