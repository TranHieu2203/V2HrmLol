...
{
    public class KpiEmployeeRepository : TLARepository<KpiEmployee>, IKpiEmployeeRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(KpiEmployeeInputDTO param)
        {
            try
            {
                ...
                // 
                var data = Map<KpiEmployeeInputDTO, KpiEmployee>(param, new KpiEmployee());
        ...
        public async Task<ResultWithError> UpdateAsync(KpiEmployeeInputDTO param)
        {
            try
            {
                ...
                var data = Map<KpiEmployeeInputDTO, KpiEmployee>(param, r);
        ...
        public async Task<ResultWithError> ExportTemplate(KpiEmployeeInput param)
        {
            ...
            var ds = QueryData.ExecuteStoreToTable("PKG_KPI.GET_EMP_KPI_IMP",
                new
                {
        ...
        public async Task<ResultWithError> CaclKpiSalary(KpiEmployeeInput param)
        {
            ...
            await QueryData.Execute("PKG_KPI.CALC_KPI_SALARY",
                  new
                  {
        ...
        public async Task<ResultWithError> ImportFromTemplate(KpiEmployeeImport param)
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                var dataImport = new List<KpiEmployeeTmp>();
                //Validate data
                var key = true;
                ...
                foreach (var item in ListData)
                {
                    ...
                    if (item.RealValue != 0)
                    {
                        ...
                        var kpiItem = new KpiEmployeeTmp();
                ...
                var r = await QueryData.ExecuteObject("PKG_KPI.IMPORT_KPI",
                   new
                   {
                       P_PERIOD_ID = param.PeriodId
                   }, true);
        ...
        public async Task<ResultWithError> LockKPI(LockInputDTO param)
        {
            try
            {
                var r = await QueryData.ExecuteObject("PKG_PAYROLL.LOCK_KPI",
                    new
                    {
                        p_is_admin = _appContext.IsAdmin == true ? 1 : 0,
