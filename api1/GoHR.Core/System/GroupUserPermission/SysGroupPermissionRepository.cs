...
{
    public class SysGroupPermissionRepository : TLARepository<SysGroupPermission>, ISysGroupPermissionRepository
    {
            ...
            foreach (var item in param)
            {
                if (item.FunctionId != 0 && item.GroupId != 0)
                {
                    if (item.PermissionString.Trim().Length > 0)
                    {
                        ...
                        {
                            // insert
                            var data = Map<GroupPermissionInputDTO, SysGroupPermission>(item, new SysGroupPermission());
