...
{
    public class AdvanceRepository : TLARepository<Advance>, IAdvanceRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(AdvanceInputDTO param)
        {
            try
            {
                ...
                //}
                var data = Map<AdvanceInputDTO, Advance>(param, new Advance());
        ...
        public async Task<ResultWithError> UpdateAsync(AdvanceInputDTO param)
        {
            try
            {
                ...
                r.STATUS_ID = 1;
                var data = Map<AdvanceInputDTO, Advance>(param, r);
        ...
        public async Task<ResultWithError> TemplateImport(AdvanceTmpParam param)
        {
            try
            {
                ...
                var ds = QueryData.ExecuteStoreToTable("PKG_IMPORT.ADVANCE_DATA_IMPORT",
                new
                {
                    P_ORG_ID = param.OrgId,
        ...
        public async Task<ResultWithError> ImportTemplate(AdvanceTmpParam param)
        {
            try
            {
                ...
                {
                    var pathTemp = _appContext._config["urlTempAdvance"];
                    var memoryStream = Template.FillTemp<AdvanceTmpDtlParam>(pathTemp, lstError);
                    return new ResultWithError(memoryStream);
                }
                else
                {
                    // xử lý fill dữ liệu vào master data
                    var ds = QueryData.ExecuteStoreToTable("PKG_IMPORT.ADVANCE_IMPORT",
                    new
                    {
                        P_REF_CODE = guid,
                        P_YEAR = param.Year,
