...
namespace PayrollDAL.Repositories
{
    public class ReportRepository : TLARepository, IReportRepository
    {
        ...
        public async Task<ResultWithError> PA002(ParaInputReport param)
        {
            try
            {
                var r =  QueryData.ExecuteStoreToTable("PKG_REPORT.RPT_PA002",
                new
                {
                    P_ORG_ID = param.OrgId,
        ...
        public async Task<ResultWithError> ThuNhapBinhQuan(int? orgId)
        {
            ...
            var lstLinq = JsonConvert.DeserializeObject<List<ThuNhapBinhQuanObject>>(JsonConvert.SerializeObject(ds.Tables[0])).AsEnumerable<ThuNhapBinhQuanObject>();
        ...
        public async Task<ResultWithError> CoCauNhanSu(CCNSParam param)
        {
            ...
            var lstLinq = JsonConvert.DeserializeObject<List<CoCauNhanSuObject>>(JsonConvert.SerializeObject(ds.Tables[0])).AsEnumerable<CoCauNhanSuObject>();
            var parentNameList = lstLinq.Select(x => x.ORG_NAME2).Distinct().ToList();
            for (int i = 0; i <= parentNameList.Count - 1; i++)
            {
                var item = parentNameList[i];
        ...
        public async Task<ResultWithError> TongHopQuyLuong(CCNSParam param)
        {
            ...
            var lstLinq = JsonConvert.DeserializeObject<List<TongHopQuyLuongObject>>(JsonConvert.SerializeObject(ds.Tables[0])).AsEnumerable<TongHopQuyLuongObject>();
            var parentNameList = lstLinq.Select(x => x.ORG_NAME2).Distinct().ToList();
            for (int i = 0; i <= parentNameList.Count - 1; i++)
            {
                var item = parentNameList[i];
