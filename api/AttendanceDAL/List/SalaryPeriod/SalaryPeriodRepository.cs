...
{
    public class SalaryPeriodRepository : TLARepository<SalaryPeriod>, ISalaryPeriodRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(SalaryPeriodInputDTO param)
        {
            try
            {
                ...
                var p = await _appContext.SalaryPeriods
                     .Where(f => (param.DateStart <= f.DATE_END && param.DateStart >= f.DATE_START) || (param.DateEnd >= f.DATE_START && param.DateEnd <= f.DATE_END))
                     .CountAsync();
                if (p > 0)
                {
                ...
                var data = Map<SalaryPeriodInputDTO, SalaryPeriod>(param, new SalaryPeriod());
                data.IS_ACTIVE = true;
                var result = await _appContext.SalaryPeriods.AddAsync(data);
                await _appContext.SaveChangesAsync();
                if (param.Dtl.Count > 0)
                {
                    var lst = new List<SalaryPeriodDtl>();
                    foreach (var item in param.Dtl)
                    {
                        var dtl = Map<SalaryPeriodDtlDTO, SalaryPeriodDtl>(item, new SalaryPeriodDtl());
        ...
        public async Task<ResultWithError> UpdateAsync(SalaryPeriodInputDTO param)
        {
            try
            {
                ...
                // check ky công lương
                var p = await _appContext.SalaryPeriods
                    .Where(f => f.ID != r.ID && ((param.DateStart <= f.DATE_END && param.DateStart >= f.DATE_START) || (param.DateEnd >= f.DATE_START && param.DateEnd <= f.DATE_END)))
                    .CountAsync();
                if (p > 0)
                {
                    return new ResultWithError("DATA_EXIST");
                }
                var data = Map<SalaryPeriodInputDTO, SalaryPeriod>(param, r);
                ...
                if (param.Dtl.Count > 0)
                {
                    ...
                    foreach (var item in param.Dtl)
                    {
                        var dtl1 = Map<SalaryPeriodDtlDTO, SalaryPeriodDtl>(item, new SalaryPeriodDtl());
        ...
        public async Task<ResultWithError> GetList(int? Id)
        {
            try
            {
                var queryable = await (from p in _appContext.SalaryPeriods
                                       ...
                                       orderby p.DATE_START descending
                                       select new
                                       {
                                           Id = p.ID,
                                           Name = p.NAME,
