...
{
    public class RegisterOffRepository : TLARepository<RegisterOff>, IRegisterOffRepository
    {
        ...
        public async Task<ResultWithError> UpdateAsync(RegisterOffInputDTO param)
        {
            try
            {
                ...
                var data = Map<RegisterOffInputDTO, RegisterOff>(param, r);
        ...
        /// <returns></returns>
        public async Task<ResultWithError> ChangeStatusAsync(List<int> ids, int StatusId)
        {
            try
            {
                ...
                if (type == Consts.REGISTER_OFF) // Đăng ký nghỉ
                {
                    var r = await (from p in _appContext.RegisterOffs
                                   ...
                                   select new
                                   {
                                       ...
                                       dateStart = p.DATE_START,
                                       dateEnd = p.DATE_END,
                                       workingDay = p.CREATE_DATE, // phiên bản cũ
            ...
            try
            {
                if (type == Consts.REGISTER_OFF)
                {
                    var r = await (from a in _appContext.Approveds
                                   ...
                                   join t in _appContext.TimeTypes on p.TIMETYPE_ID equals t.ID
                                   where p.ID == id
                                   orderby a.STATUS_ID ascending, a.CREATE_DATE descending
                                   select new
                                   {
                                       Id = p.ID,
                                       FromDate = p.DATE_START,
        ...
        public async Task<PagedResult<HistoryRegDTO>> GetRegister(Pagings param)
        {
            try
            {
                var queryable = (from p in _appContext.RegisterOffs
                                 ...
                                 orderby p.STATUS_ID, p.CREATE_DATE descending
                                 select new HistoryRegDTO
                                 {
                                     Id = p.ID,
                                     Date = p.CREATE_DATE, // build xong IOS bỏ
                                     DateStart = p.DATE_START,
                                     DateEnd = p.DATE_END,
                                     Status = p.STATUS_ID,
                                     TypeId = p.TYPE_ID
        ...
        private async Task<ResultWithError> Register(RegisterOffInputDTO param, int type, long? empId)
        {
            try
            {
                ...
                if (type == Consts.REGISTER_OFF)
                {
                    ...
                    data = Map<RegisterOffInputDTO, RegisterOff>(param, new RegisterOff());
                }
                    ...
                    if (Mng.mngId == app.EMP_RES_ID) // người phê duyệt là người đăng ký Up lên 1 lvl
                    {
                        ...
                        "PKG_NOTIFI.GET_FMC_TOKEN", new
                        {
                            ...
                            if (lstDevice.Count > 0)
                            {
                                ...
                                no.CREATE_DATE = DateTime.Now;
                                // sau khi lưu gửi thông báo
                                var FullName = _appContext.Employees.Where(x => x.ID == data.EMPLOYEE_ID).Select(e => e.FULLNAME).FirstOrDefault();
                                var Title = "";
                                var Body = "";
                                if (type == Consts.REGISTER_OFF)
                                {
