...
{
    public class ReportRepository : TLARepository<Report>, IReportRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(ReportInputDTO param)
        {
            try
            {
                ...
                param.Id = null;
                var data = Map<ReportInputDTO, Report>(param, new Report());
        ...
        public async Task<ResultWithError> ReportIns(ReportInsInputDTO param)
        {
            try
            {
                var data = await QueryData.ExecuteList("PKG_REPORT.REPORT_INS", new
                {
        ...
        public async Task<ResultWithError> ReportInsByOrg(ReportInsByOrgDTO param)
        {
            try
            {
                var data = await QueryData.ExecuteList("PKG_REPORT.REPORT_INS_PERIOD", new
                {
        ...
        public async Task<PagedResult<ReportEmployeeDTO>> ReportEmployee(ReportEmployeeDTO param)
        {
            try
            {
                var queryable = from p in _appContext.Employees
                                ...
                                join ot in _appContext.OtherLists on p.LEARNING_LEVEL_ID equals ot.ID into tmp2
                                from ot2 in tmp2.DefaultIfEmpty()
                                join g in _appContext.OtherLists on p.GENDER_ID equals g.ID into tmp3
                                ...
                                select new ReportEmployeeDTO
                                {
                                    ...
                                    Language = p.LANGUAGE,
                                    Qualification = p.QUALIFICATION_ID,
                                    Gender = g2.NAME,
