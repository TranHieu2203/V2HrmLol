...
{
    public class InsChangeRepository : TLARepository<InsChange>, IInsChangeRepository
    {
        ...
        public async Task<ResultWithError> CreateAsync(InsChangeInputDTO param)
        {
            try
            {
                ...
                var data = Map<InsChangeInputDTO, InsChange>(param, new InsChange());
        ...
        public async Task<ResultWithError> UpdateAsync(InsChangeInputDTO param)
        {
            try
            {
                ...
                var data = Map<InsChangeInputDTO, InsChange>(param, r);
        ...
        /// <returns></returns>
        public async Task<ResultWithError> RemoveAsync(List<int> ids)
        {
            try
            {
        ...
        public async Task<ResultWithError> TemplateImport(int orgId)
        {
            try
            {
                ...
                var ds = QueryData.ExecuteStoreToTable("PKG_IMPORT.INSURANCE_DATA_IMPORT",
                new
                {
                    P_ORG_ID = orgId,
        ...
        public async Task<ResultWithError> ImportTemplate(ImportInsParam param)
        {
            try
            {
                ...
                {
                    var pathTemp = _appContext._config["urlTempInsurance"];
                    var memoryStream = Template.FillTemp<ImportInsDtlParam>(pathTemp, lstError);
                    return new ResultWithError(memoryStream);
                }
                else
                {
                    if (lst.Count > 0)
                    {
                        ...
                        await _appContext.SaveChangesAsync();
                        // xử lý fill dữ liệu vào master data
                        var ds = QueryData.ExecuteStoreToTable("PKG_IMPORT.INSURANCE_IMPORT",
                        new
                        {
                            P_REF_CODE = guid,
                            P_CUR = QueryData.OUT_CURSOR
