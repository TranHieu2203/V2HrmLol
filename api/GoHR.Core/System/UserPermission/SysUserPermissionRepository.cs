...
{
    public class SysUserPermissionRepository : TLARepository<SysUserPermission>, ISysUserPermissionRepository
    {
            ...
            foreach (var item in param)
            {
                if (item.FunctionId != 0 && item.UserId != null)
                {
                    if (item.PermissionString.Trim().Length > 0)
                    {
                        ...
                        {
                            // insert
                            var data = Map<UserPermissionInputDTO, SysUserPermission>(item, new SysUserPermission());
        ...
        public async Task<PagedResult<GridFunctionOutput>> GridPermission(GridFunctionInput param)
        {
            try
            {
                var queryable = (from p in _appContext.SysFunctions
                                 join a in _appContext.TenantFunctions on p.ID equals a.FUNCTION_ID
                                 ...
                                 join f in _appContext.AspUserPermissions.Where(c => c.USER_ID == param.UserId) on a.FUNCTION_ID equals f.FUNCTION_ID into tmp1
