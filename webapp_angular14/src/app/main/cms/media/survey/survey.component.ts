...
export class SurveyComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/cms/media/survey", paramAdd]);
  };

  getListData = (): void => {
    const state = { skip: 0, take: 20 };
    this._coreService.execute(state, "hr/Vote/GetAll");
  };
  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "hr/Vote/GetAll", extraParams);
  }
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/media/survey/new"]);
        break;
